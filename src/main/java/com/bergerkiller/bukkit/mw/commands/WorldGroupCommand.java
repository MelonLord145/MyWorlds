        ...
        if (args.length > 2) {
            if (args[0].equalsIgnoreCase("create")) {
                if (WorldGroup.get(args[1]) == null) {
                	...
                	if (world != null) {
                		WorldGroup.create(args[1], world).add(world);;
            	...
            	if (group != null) {
            		if (group.hasWorld(args[2])) {
            			if (!group.getDefaultWorld().equals(args[2])) { 
            				...
            				message(ChatColor.YELLOW + "Cannot remove world '" + args[2] + "' as it is the default world for the group '" + args[1] + "' set a new one with /world group setdefaultworld " + args[1] + " [world] then remove this world");
            			}
            		}
            		else {
